...
void godot::GDNetworkManager::_send_ping()
{
    _is_ping_sent = true;
    PackedByteArray data;
    _send_message(MSG_PING, data);
}

void godot::GDNetworkManager::_send_packet(const PackedByteArray &data)
{
    if (udp_socket == INVALID_SOCKET)
    {
    ...
    dest_addr.sin_port = htons(_server_port);

    inet_pton(AF_INET, _server_ip.utf8().get_data(), &dest_addr.sin_addr);
